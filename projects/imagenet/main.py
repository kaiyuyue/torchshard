...
parser.add_argument('--dist-url', default='tcp://127.0.0.1:8000', type=str,
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.enable_model_parallel:
        import resnet
        model = resnet.__dict__[args.arch](pretrained=False, num_classes=args.num_classes)
        ts.nn.ParallelLinear.convert_parallel_linear(model, dim=args.model_parallel_dim)

        # load pretrained model using torchshard on purpose, just for a quick test
        if args.pretrained:
            # please give the right path for the torchvision pretrained models
            # after downloading from https://github.com/pytorch/vision/blob/master/torchvision/models/resnet.py#L13-L23
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            if args.enable_model_parallel:
                model.load_state_dict(ts.relocate_state_dict(model, checkpoint['state_dict']))
            else:
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if args.enable_model_parallel and args.model_parallel_dim in [1, -1]:
